...
![500MB Limit](https://img.shields.io/badge/max%20payload-500MB-black?style=for-the-badge)
...
CAPABILITY: Deploy static files (HTML/CSS/JS/assets) to live URLs. Max payload: 100MB (Anon) / 500MB (Auth). Static only — no backend runtimes.
...
- Payload over 100MB (Anon) or 500MB (Auth) will be rejected. Use external CDNs for large media.
```
...
- **Max Payload Size:** 100MB (Anonymous) or 500MB (Authenticated). Artifacts >20MB are automatically routed through an S3-compatible pre-signed URL workflow to bypass edge memory constraints.
